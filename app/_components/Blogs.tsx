    ...
    <div className="container  ">
      ...
      <div className="flex gap-4 mt-5 max-lg:overflow-x-scroll max-lg:px-2 no-scrollbar">
        {blogs.map((el, i) => (
          // <div
          //   key={i}
          //   className="rounded-md rounded-tl-2xl rounded-tr-2xl bg-white flex flex-col gap-2 pb-3  max-lg:w-[85%] max-lg:shrink-0"
          // >
          ...
          //   </div>
          // </div>
          <BlogCard key={i} blog={el} location="Homepage" />
        ))}
      </div>
    </div>
  );
